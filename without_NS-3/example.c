...
void example_kmedoids(int nrows, int ncols, double** distmatrix, int** mask)
/* Perform k-medoids clustering on genes */
{ 
  ...
  double error;
  int ifound = 0;
  kmedoids(nclusters, nelements, distmatrix, npass, clusterid, &error, &ifound);
  printf ("Solution found %d times; within-cluster sum of distances is %f\n",
    ifound, error);
  printf ("Cluster assignments:\n");
  ...
  // int *centroids = (int *)malloc(sizeof(int)*nclusters);
  // double *errors = (double*)malloc(sizeof(double)*nclusters);
  // for(int i=0;i<nclusters;i++){errors[i] = 1.0;}
  // getclustermedoids(nclusters, nelements, distmatrix, clusterid, centroids, errors);


  // printf("-------CLUSTER MEDOIDS-----------\n");
  // printf("ClusterID   MedoidID\n");
...
// Compute the distance matrix using data and mask matrices
double** compute_distance_matrix(int nrows,int ncols,double** data,int** mask)
{
  double ** distmatrix_temp = (double**)malloc(sizeof(double*)*nrows);
  for(int i=0;i<nrows;i++)
  {
    distmatrix_temp[i] = (double*) malloc(sizeof(double)*nrows);
  }
  ...
  for(int i=0;i<nrows;i++)
  {
    for(int j=0;j<nrows;j++)
    {
      ...
      {
        if(mask[i][k]==1 && mask[j][k]==1)
        {
          dist += pow((data[i][k] - data[j][k]), 2);
        }
        else if(mask[i][k]==1 && mask[j][k]==0)
        {
          dist += pow((data[i][k]), 2);
        }
        else if(mask[i][k]==0 && mask[j][k]==1)
        {
          dist += pow((data[j][k]),2);
        }
        else
      ...
      distmatrix_temp[i][j] = sqrt(dist);
    }
  }
  return distmatrix_temp;
}
/* ========================================================================= */
...
{ int i;
  ...
  data[ 1][ 0]=0; data[ 1][ 1]=4;
  data[ 2][ 0]=4; data[ 2][ 1]=0;
  data[ 3][ 0]=2; data[ 3][ 1]=2;
  data[ 4][ 0]=4; data[ 4][ 1]=4;
  ...
  // Calculate distance matrix again
  
  double** distmatrix_temp = compute_distance_matrix(nrows, ncols, data, mask);

  example_kmedoids(nrows, ncols, distmatrix_temp, mask);
